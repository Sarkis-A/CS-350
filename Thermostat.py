    ...
    def updateLights(self):
        ...
        if self.current_state.id == 'heat':
            if temp < self.setPoint:
                # When heating and below set point, fade the red LED.
                redLight.pulse()
            else:
                # When heating and at or above set point, keep the red LED on solid.
                redLight.on()
        elif self.current_state.id == 'cool':
            if temp > self.setPoint:
                # When cooling and above set point, fade the blue LED.
                blueLight.pulse()
            else:
                # When cooling and at or below set point, keep the blue LED on solid.
                blueLight.on()
        else:
            # In the off state, ensure both LEDs remain off.
            redLight.off()
            blueLight.off()
...
##
## Configure our Green button to use GPIO 25 and to execute
## the function to increase the setpoint by a degree.
##
greenButton = Button(25)
greenButton.when_pressed = tsm.processTempIncButton
